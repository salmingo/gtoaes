...
protected:
	...
	struct ntp_packet {
		...
		int root_dispersion;
		/*!
		 * @brief in the case of stratum 0 or 1, this is a four-octet, left-justified
...
public:
	typedef boost::shared_ptr<NTPClient> Pointer;
...
public:
	 ...
	 */
	NTPClient(const char* hostIP, const uint16_t port = 123, const int tSync = 100);
	virtual ~NTPClient();
...
public:
	static Pointer Create(const char* hostIP, const uint16_t port = 123, const int tSync = 100) {
		return Pointer(new NTPClient(hostIP, port, tSync));
	}
...
typedef NTPClient::Pointer NTPPtr; ///< NTPclient指针
